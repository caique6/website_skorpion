...
import { DonationContent, DonationStatus } from "../types";
...
export const DonationSuccess = ({ content, status, onReset }: Props) => {
      ...
      <div className="flex flex-col gap-3">
        <motion.div
          ...
          className={`flex h-14 w-14 items-center justify-center rounded-full ${
            rejected ? "bg-red-500/10" : "bg-green-500/10"
          }`}
        >
          {rejected ? (
            <XCircle className="h-7 w-7 text-skorpion-red" strokeWidth={2} />
          ) : (
